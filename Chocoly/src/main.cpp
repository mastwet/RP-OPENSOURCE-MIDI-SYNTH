...
char menu_selected_item[4] = {0};
int selectedIndex = 0; // 当前选中的索引

unsigned char current_keypad_setting_num = 0;
...
// 绘制右侧内容，根据不同的选择显示不同的内容
    if (selectedIndex == 0) {
        ...
        u8g2.print("KEY Page:1");

        // 绘制 4x4 的格子
        int gridSize = 10;
        for (int row = 0; row < 4; row++) {
            for (int col = 0; col < 4; col++) {
                int index = row * 4 + col; // 计算格子的索引
                if (index == menu_key_selected) { // 如果当前格子的索引与选定的相同
                    u8g2.drawBox(30 + col * gridSize, 20 + row * gridSize, gridSize, gridSize); // 实心填充
                } else {
                    u8g2.drawFrame(30 + col * gridSize, 20 + row * gridSize, gridSize, gridSize); // 空心框
                }
            }
        }
        ...
        u8g2.print("KNOB Page:2");

        // 绘制四个圆圈，及其参数
        int circleRadius = 4;
        for (int i = 0; i < 4; i++) {
            if (i == menu_knob_selected) { // 如果当前圆圈的索引与选定的相同
                u8g2.drawDisc(32 + i * 16, 30, circleRadius); // 实心填充
            } else {
                u8g2.drawCircle(32 + i * 16, 30, circleRadius); // 空心圆
            }
        }
        ...
        u8g2.print("CVALUE: 20");
        u8g2.setCursor(30, 64);
        u8g2.print("CCHANNEL: 30");
    }
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (i == menu_button_selected) { // 如果当前方形的索引与选定的相同
                u8g2.drawBox(x, y, boxSize, boxSize); // 实心填充
            } else {
                u8g2.drawFrame(x, y, boxSize, boxSize); // 空心框
            }
        }

        u8g2.drawFrame(98, 33, 23, 13);
        ...
        u8g2.print("C NUM: 5");
        u8g2.setCursor(30, 56);
        u8g2.print("CVALUE: 15");
        u8g2.setCursor(30, 64);
        u8g2.print("CCHANNEL: 25");
    }
        ...
        u8g2.print("Bluetooth: OFF");
        u8g2.setCursor(30, 32);
        u8g2.print("OTA Connect");
        u8g2.setCursor(30, 42);
        u8g2.print("Init Device");
...
void handle_button_menu_event(int num, int event) {
    ...
    if (event == BUTTON_PRESSED_EVENT) {
        switch (num) {
            ...
            case 1:
                if(selectedIndex == 1){
                    menu_knob_selected = 0;
                }
                else if(selectedIndex == 2){
                    menu_button_selected = 0;
                }
                break;
            case 5:
                if(selectedIndex == 1){
                    menu_knob_selected = 1;
                }
                else if(selectedIndex == 2){
                    menu_button_selected = 1;
                }
                break;
            case 4:
                if(selectedIndex == 1){
                    menu_knob_selected = 2;
                }
                else if(selectedIndex == 2){
                    menu_button_selected = 2;
                }
                break;
            case 0:
                if(selectedIndex == 1){
                    menu_knob_selected = 3;
                }
                else if(selectedIndex == 2){
                ...
                break;
            case 7:  // 按钮1：选择设置PAD
                selectedIndex = 0;  // 设置菜单模式为1表示设置PAD
                printf("Menu mode: Set PAD\n");
                break;
            case 6:  // 按钮2：选择设置编码器
                selectedIndex = 1;  // 设置菜单模式为2表示设置编码器
                printf("Menu mode: Set Encoder\n");
                break;
            case 2:  // 按钮3：选择设置按钮
                selectedIndex = 2;  // 设置菜单模式为3表示设置按钮
                printf("Menu mode: Set Button\n");
                break;
            case 3:
                selectedIndex = 3;
                printf("Menu mode: into settings\n");
                break;
            default:
...
void handle_keypad_menu_event(int num, int event) {
    if(selectedIndex == 0){
        if(event == KEY_PRESSED_EVENT){
            menu_key_selected = num;
        }
    }
}
        ...
        if(event_temp.type != -1){
            ...
            //处理键盘矩阵事件
                ...
                if(!menu_flag) {
                    ...
                    handle_keypad_menu_event(event_temp.num, event_temp.event);
                }
            }
            //处理编码器事件
