...
void TM1650::init() {
	...
	Wire1.beginTransmission(TM1650_DISPLAY_BASE);
	iActive = (Wire1.endTransmission() == 0);
	clear();
	displayOn();
}
...
void TM1650::setBrightness(unsigned int aValue) {
	...
	for (unsigned int i=0; i<iNumDigits; i++) {
		Wire1.beginTransmission(TM1650_DCTRL_BASE+(int)i);
		iCtrl[i] = (iCtrl[i] & TM1650_MSK_BRIGHT) | ( iBrightness << TM1650_BRIGHT_SHIFT );
		Wire1.write((byte) iCtrl[i]);
...
void TM1650::controlPosition(unsigned int aPos, byte aValue) {
	if (!iActive) return;
	if (aPos < iNumDigits) {
		Wire1.beginTransmission(TM1650_DCTRL_BASE + (int)aPos);
		iCtrl[aPos] = aValue;
		Wire1.write(aValue);
		Wire1.endTransmission();
	}
...
void TM1650::setPosition(unsigned int aPos, byte aValue) {
	if (!iActive) return;
	if (aPos < iNumDigits) {
		Wire1.beginTransmission(TM1650_DISPLAY_BASE + (int)aPos);
		iBuffer[aPos] = aValue;
		Wire1.write(aValue);
		Wire1.endTransmission();
	}
...
void TM1650::clear() {
	if (!iActive) return;
	for (unsigned int i=0; i<iNumDigits; i++) {
		Wire1.beginTransmission(TM1650_DISPLAY_BASE+(int)i);
		iBuffer[i] = 0;
		Wire1.write((byte) 0);
		Wire1.endTransmission();
	}
...
void TM1650::displayChar(byte aPos, byte aData, bool aDot) {
	...
	iBuffer[aPos] = aData;
	Wire1.beginTransmission(TM1650_DISPLAY_BASE + aPos);
	Wire1.write(aData);
	Wire1.endTransmission();
}
...
byte TM1650::getButtons(void) {
	byte keys = 0;
	Wire1.requestFrom(TM1650_DCTRL_BASE, 2);
	keys = Wire1.read();
	Wire1.read();
	return keys;
}